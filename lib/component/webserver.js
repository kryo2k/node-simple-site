...
(function(util, events, _, http, paperboy){
    ...
    _.extend(Webserver, {
        PaperboyServer: function(req, res){
            ...
            var
            con = req.connection,
            l_address = con.address(),
            self = instance,
            serverReqMsg = function(status,send) {
                var
                local = [l_address.address, l_address.port],
                remote = [con.remoteAddress,con.remotePort],
                splice = null,
                args = [req.url, status];

                if( send ) {
                    splice = local.concat(remote);
                }
                else {
                    splice = remote.concat(local);
                }

                util.insertArrayAt(args, 0, splice);

                return util.format.apply(util, ["(%s:%d to %s:%d) [%s]: %s"].concat(args));
            },
            tStart = new Date();
            ...
            paperboy.deliver(webroot, req, res)
                ...
                .after(function(statusCode) {
                    self.logInfo(serverReqMsg(httpmsg(statusCode) + util.format(' (%s)',util.formatBenchmark(tStart)),true));
                })
                .error(function(statusCode, msg) {
                    self.logWarn(serverReqMsg(httperrorf(statusCode,req.url),true));
                    self.logDebug(msg);
                ...
                .otherwise(function(err) {
                    var code = 404;
                    self.logWarn(serverReqMsg(httperrorf(code,req.url),true));
                    self.logDebug(err);
    ...
    _.extend(Webserver.prototype, {
        ...
        start: function(callback) {
            var
            self = this;
            self.logInfo('server starting');

            if( self.isDebugging() ) {
                self.logDebug('host:',util.format('%s:%d', addr, port));
                self.logDebug('webroot:',webroot);
                self.logDebug('poweredby:',poweredby);
            }
            ...
            if( self.isDebugging() ) {
                server.on('listening',function(){
                    var
                    l_addr = server.address();
                    self.logDebug(util.format('server listening on %s:%d',l_addr.address,l_addr.port));
                });
                server.on('connection',function(sock){
                    var
                    l_addr = sock.address(),
                    r_addr = sock.remoteAddress,
                    r_port = sock.remotePort,
                    c_start = new Date();

                    self.logDebug(util.format('new connection from %s:%d on %s:%d',r_addr,r_port,l_addr.address,l_addr.port));

                    sock.on('close',function(){
                        self.logDebug(util.format('connection from %s:%d closed (duration: %s)',r_addr,r_port, util.formatBenchmark(c_start)));
                    });
                });
                server.on('close',function(){
                    self.logDebug('server stopped');
                });
            }

            server.listen(port, addr, undefined, function(){
        ...
        stop: function(callback) {
            var
            self = this;
            self.logInfo('server is stopping');

            server.close(function(){
                server = null;
            });

            util.callback(callback); // Weird bug in nodejs http server, doesn't trigger callback on server.close();
        },
